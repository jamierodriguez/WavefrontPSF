...
parser.add_argument('--options', dest='options', default=0, type=int,
...
fitname_option = ['empty']

fitname_option.append('e1e2_normalparams')
if option_int == 1:
...
fitname_option.append('e0_normalparams')
if option_int == 2:
...
fitname_option.append('deltas_normalparams')
if option_int == 3:
...
fitname_option.append('zetas_normalparams')
if option_int == 4:
...
fitname_option.append('e1e2_fewerparms')
if option_int == 5:
...
fitname_option.append('e0_fewerparams')
if option_int == 6:
...
fitname_option.append('deltas_fewerparams')
if option_int == 7:
...
fitname_option.append('zetas_fewerparams')
if option_int == 8:
...
fitname_option.append('es_allparams')
if option_int == 9:
...
def generate_directories(expid, option_int):

    fitname = 'fits' + daycode + '/' + fitname_option[option_int]

    catalog_directory = catalog_directory_func(expid)
    run_directory = path.join(path.join(base_catalog_directory, tag), fitname + '/')
    log_directory = path.join(path.join(base_catalog_directory, tag), 'logs' + '/fits' + daycode + '/')
    ...
    return fitname, run_directory, log_directory, catalog_directory, fits_directory, fits_afterburn_directory

fitname, run_directory, log_directory, catalog_directory, fits_directory, fits_afterburn_directory = generate_directories(expid, option_int)
###############################################################################
# master driver
###############################################################################
...
if (driver == 'run_master') + (driver == 'run_master_afterburn'):
    if option_int != 0:
        option_ints = [option_int]
    else:
        option_ints = range(1, len(fitname_option))
    for expid in expids:
        for option_int in option_ints:
            ...
            jobname = '{1}_{2}_{0:08d}'.format(expid, driver, fitname_option[option_int])
            args_input = {
                          ## 'driver': 'run_catalog',
                          ## 'driver': 'run_expid',
                          'driver': 'run_catalog_and_expid',
                          'expid': expid,
                          'options': option_int,
                          }
            if shell == cpd_shell:
                ...
                if driver == 'run_master':
                    queue = 'medium'
                    args_input['driver'] = 'run_catalog_and_expid'
                elif driver == 'run_master_afterburn':
                    queue = 'long'
                    args_input['driver'] = 'run_catalog_and_expid_and_afterburn'
                command = [
                    'bsub',
                    #'-W', '120',
                    '-q', queue,
...
if driver == 'merge_results':
    ...
    if option_int != 0:
        ...
        option_ints = range(1, len(fitname_option))
    for option_int_i in option_ints:
        print('doing option', option_int_i)

        fitname, run_directory, log_directory, catalog_directory, fits_directory, fits_afterburn_directory = generate_directories(expid, option_int_i)
        fits_dict = {'expid': []}
        for expid in expids:
        ...
        # save results
        np.save(run_directory + 'results_dict', fits_dict)
        try:
            # convert dictionary
