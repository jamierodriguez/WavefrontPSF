...
# repeat for sextractor catalogs
# find the locations of the catalog files
list_catalogs, list_fits_extension, list_chip = \
        generate_hdu_lists_sex(args_dict['expid'], args_dict['catalogs'])


FP_sex = FocalPlane(list_catalogs=list_catalogs,
                list_fits_extension=list_fits_extension,
                list_chip=list_chip,
                path_mesh=args_dict['path_mesh'],
                mesh_name=args_dict['mesh_name'],
...
figures, axes, scales = data_focal_plot(FP_sex.data, boxdiv=FP_sex.boxdiv,
                                        average=FP_sex.average)
for figure_key in figures:
    if figures[figure_key]:
        figures[figure_key].savefig(args_dict['output_directory'] +
                                    'DECam_{0:08d}_'.format(args_dict['expid']) +
                                    'sex_focal_{0}.png'.format(figure_key))
close('all')

edges = FP_sex.decaminfo.getEdges(FP_sex.boxdiv)
figures, axes, scales = data_hist_plot(FP_sex.data_unaveraged, edges=edges)
for figure_key in figures:
    if figures[figure_key]:
        figures[figure_key].savefig(args_dict['output_directory'] +
                                    'DECam_{0:08d}_'.format(args_dict['expid']) +
                                    'sex_hist_{0}.png'.format(figure_key))
