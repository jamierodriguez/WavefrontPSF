...
void DonutEngine::calcParameters(int iT){
  ...
  if (iT==0) {
    ...
    _outerRadius = 1.10874;
    _innerRadius = 0.58 * _outerRadius;
    _zLength = 2.734706;
    _lambdaz = _waveLength * _zLength;  
    _fLength = 10.31007;
    _pixelSize = 10.0e-6;
...
void DonutEngine::calcWFMtoImage(double* par, int n){  
  calcWFMtoImage(par);
}
 
void DonutEngine::calcWFMtoImage(Real* par){
      
  // fill parameter and determine how much of the calculation to repeat
  // except just start from WFM...
  fillPar(par);
...
void DonutEngine::calcPsfToImage(double* par, int n){
  calcPsfToImage(par);
}

void DonutEngine::calcPsfToImage(Real* par){
      
  // fill parameter and determine how much of the calculation to repeat
  // start from psf get full pixelated imaget
  fillPar(par);
...
void DonutEngine::calcDerivatives(Real* image, Real* weight){
  ...
  _dChi2dpar[ipar_rzero] = 0.0;  // we don't calculate it!
  for (int iZ=0;iZ<nZernikeSize;iZ++){
