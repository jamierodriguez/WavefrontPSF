...
def check_centered_peak(stamp_in,
    ...
    stamp = stamp.reshape(args['size'], args['size'])

    # do window
    max_nsig2 = 25
    y, x = np.indices(stamp.shape)
    Mx = args['size'] / 2 + xwin_image % int(xwin_image) - 1
    My = args['size'] / 2 + ywin_image % int(ywin_image) - 1
...
def catalog(args):
    ...
    ##############################################################################
    list_catalogs_base = \
        path.expandvars(args['output_directory'])
    
    if args['all'] > 0:
        download_desdm(args['expid'], list_catalogs_base, ccd=None)
    else:
        download_desdm_filelist(args['expid'], list_catalogs_base, ccd=None)
    ...
    for i in xrange(1, 63):
        ...
        # get cat and image
        if args['all'] == 0:
            download_desdm(args['expid'], list_catalogs_base, ccd=i)
    
        ## # get cat
        ## download_cat(args['rid'], args['expid'], args['date'], i)
        ## # get image
        ## download_image(args['rid'], args['expid'], args['date'], i)
        ...
        for recdata in FP.recdata:
            ...
            y_start = int(y_center - args['size'] / 2)
            y_end = y_start + args['size']
            x_start = int(x_center - args['size'] / 2)
            x_end = x_start + args['size']
        ...
        tbhdulist.writeto(list_catalogs_base + \
                          'DECam_{0:08d}_'.format(args['expid']) + \
        ...
        if i != 1:
            ...
            remove(list_catalogs_base +
                   "DECam_{0:08d}_{1:02d}.fits.fz".format(args['expid'], i))
