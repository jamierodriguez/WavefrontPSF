...
#from ROOT import TStyle,gStyle,TCanvas,TGraph3D
...
class PointMesh(object):
    ...
    def readPointsFromFile(self,fileName):
        ...
        # read in pointsArray from file
        dataPoints = numpy.loadtxt(fileName,dtype=[('Sensor','|S3'),('x','float'),('y','float'),('z','float'),('w','float')])
        ...
        # now convert lists to numpy arrays, and refill new dict
        self.pointsArray = {}
        for key in pointsDict:
            aList = pointsDict[key]
            anArr = numpy.array(aList)
            self.pointsArray[key] = anArr
    

    def __init__(self,coordList,gridArray,pointsArray=None,pointsFile=None,myMethod='sbs',debugFlag=False):
        ...
        if self.myMethod=='sbs' or self.myMethod=='rbf' or self.myMethod=='tmean' or self.myMethod=='grid' or self.myMethod == "idw":
            ...
            self.makeInterpolation(self.myMethod)

    def __getstate__(self):
        stateDict = {}
        keysToPickle = ['nCoord','gridArray','radiusVignetted','pointsArray','myMethod','coordList','debugFlag']
    ...
    def makeInterpolation(self,myMethod="sbs"):
        ...
        for iCoord in self.coordList:
            ...
            yGrid,xGrid = self.makeGrid(ny,ylo,yhi,nx,xlo,xhi)
            ...
            if npts>=5:
                ...
                if myMethod =="sbs":
                    ...
                    self.interpIDW[iCoord] = IDWInterp(xData,yData,zData,epsilon=1.0)
